...
export default async function handler(req, res) {
  ...
  try {
    const processedAppRegistry = baseAppConfig.map(appConfig => {
      ...
      console.log(`Stored key for keyId: ${keyId} (expires in ${KEY_EXPIRY_MS / 1000}s)`);


      // Remove htmlPath and add encrypted content and keyId
      const { htmlPath, ...restOfAppConfig } = appConfig;
      return {
        ...restOfAppConfig,
        encryptedHtml,
        keyId,
    ...
    res.status(200).json(processedAppRegistry.filter(app => !app.error));
  } catch (e) {
    console.error("Error processing app registry:", e);
    res.status(500).json({ error: 'Failed to process app registry.' });
  }
}

// This function might be called by api/login.js or a new endpoint
export function getDecryptionKeys(keyIds) {
  ...
  for (const keyId of keyIds) {
    ...
    if (cached && (now - cached.timestamp <= KEY_EXPIRY_MS)) {
      ...
      // tempKeyCache.delete(keyId); // Optionally delete after dispensing if one-time use
    } else if (cached) {
      // Key expired
      tempKeyCache.delete(keyId);
      console.log(`Attempt to use expired keyId: ${keyId}`);
    }
  }
  return keys;
}
